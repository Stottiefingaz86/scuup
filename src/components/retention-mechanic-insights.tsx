      ...
      <CardContent className="flex flex-col gap-3">
        {!ownLoggedIn ? (
          <p className="rounded-lg border border-score-mid/30 bg-score-mid/[0.06] px-3 py-2 text-xs leading-relaxed text-muted-foreground">
            ...
            can be wrong when the feature already exists behind login.
          </p>
        ) : null}
        {insights
          .filter((i) => i.requires !== "tracked_play")
          .map((insight) => {
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                      {insight.screenshotUrl ? (
                        <ScreenshotLightbox
                          ...
                          alt={`Evidence: ${insight.label}`}
                          caption={`${ownBrand.name}: ${insight.label}`}
                          className="aspect-[8/5] w-full shrink-0 sm:w-40"
                        />
                      ) : null}
                          ...
                          {insight.bestCompetitor && behind ? (
                            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                              ...
                              {insight.competitorNote
                                ? ` — ${insight.competitorNote}`
                                : " — study their loyalty hub in the offers above."}
                            </p>
                          ) : null}
                        </div>
                      </div>
