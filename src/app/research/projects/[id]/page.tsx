...
function StageEvidenceGallery({
  stages,
  emails = [],
  watchingInbox = false,
  acquisitionSource = null,
}: {
  stages: import("@/lib/research/types").JourneyStageResult[];
  emails?: import("@/lib/research/types").EmailWatchItem[];
  ...
  type FlowItem =
    { kind: "stage"; stage: (typeof withShots)[number] } | { kind: "inbox" };
  ...
  // Every card shares one anatomy so the row lines up:
  // header (label · meta) → hero frame (h-32) → film strip (h-12) → footer.
  const cardCls =
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))]">
        {items.map((item) => {
          ...
          const s = item.stage;
          // Deposit confirmation is proven by the brand's email when there is
          // one — that is the only evidence worth showing; the screens the
          // agent happened to be on while waiting are noise.
          const confirmEmail =
            s.stageId === "deposit_confirmation"
          ...
          // Registration: last frame is the page after submit (homepage).
          // Do not splice in chat-only welcome shots — those enlarge Paul.
          const hero =
...
/**
 * First touch isn't a screen — show the channels that feed the landing page,
 * every line flowing into it. No channel is singled out: the run is a proxy
 * for all of them.
 */
function FirstTouchFunnel({ height }: { height: number }) {
  const rows = FIRST_TOUCH_CHANNELS;
  // Fixed-width stage centred in the card so SVG lines and HTML nodes agree.
  const W = 196;
  const PAD = 12;
  const rowY = (i: number) =>
